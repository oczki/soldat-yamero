...
const
	MinPlayers_Spawn = 6;    // anti-spawn will work if there are at least X players. setting it lower than 6 makes no real sense
	...
	AFK_Spec = false;  // true - moves the player to spectator team. false - kicks him out
...
begin
	...
	for i := 1 to 32 do if (getplayerstat(i, 'Active') = true) and (getplayerstat(i, 'Alive') = true) then begin
		...
		if (ticks mod 180 = 0) then anticamp(i); // once per 3 seconds - no need to check more often, and it uses up less resources
	end;
end;
...
procedure onmapchange(newmap: string);  // forgive everyone after map change
var i: byte;
begin
	getspawnpoints();
